...
const Heart = props => {
  ...
  const saveData = () => {
    ...
    formdata.append("heartrate", value)
    formdata.append("latest_info", 1)
    formdata.append("userid", _user.userDetails._id); //repost
    formdata.append("unit", 'bpm');
    props.saveUserMedicalData(formdata, 'heartrateinfo', props.type)
  }
          ...
          <ScrollView>
            <View style={{ flex: 12, flexDirection: "column", padding: 10, }}>
              ...
              <View style={{ flex: 1, marginTop: 20, flexDirection: "row", alignContent: "center", alignItems: "center", justifyContent: "center" }}>
                <Text style={{ fontSize: 16 }}>{value} bpm</Text>
              </View>
                  ...
                  renderItem={itemData => (
                    <>
                      <View style={{ flex: 1, flexDirection: 'row' }}>
                        ...
                        <Text style={{ flex: 1, fontSize: 14 }}>{itemData.item.heartrate} {itemData.item.unit}</Text>
                      </View>
                    </>
                  )}
                />
