...
const HealthLandingShopByCat = props => {
  return (
    <>
      <BaseSafeAreaView style={{ backgroundColor: props.backgroundColor }}>
        ...
        <TouchableOpacity
          ...
          >
            ...
            {props.item.item.image && props.item.item.image.src.includes(".svg") ?
              <SvgUri
                style={{ width: '64', height: 64 }}//, borderRadius: 30
                uri={props.item.item.image.src}
              />
              :
              <Image
                source={{ uri: props.item.item.image?.src.replace('http://', 'https://') }}
                style={{ width: 64, height: 64, }}//borderRadius: 30 
              />
            }
            <View
...
export default HealthLandingShopByCat;
