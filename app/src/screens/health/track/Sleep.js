...
const Sleep = props => {
  ...
  const saveData = () => {
    ...
    formdata.append("sleep", value)
    formdata.append("latest_info", 1)
    formdata.append("userid", _user.userDetails._id); //repost
    formdata.append("unit", 'hours');
    props.saveUserMedicalData(formdata, 'sleepinfo', props.type)
  }
          ...
          <ScrollView>
            <View style={{ flex: 12, flexDirection: "column", padding: 10, }}>
              ...
              <View style={{ flex: 1, marginTop: 20, flexDirection: "row", alignContent: "center", alignItems: "center", justifyContent: "center" }}>
                <Text style={{ fontSize: 16 }}>{value} Hours</Text>
              </View>
                  ...
                  renderItem={itemData => (
                    <>
                      <View style={{ flex: 1, flexDirection: 'row' }}>
                        ...
                        <Text style={{ flex: 1, fontSize: 14 }}>{itemData.item.sleep} {itemData.item.unit}</Text>
                      </View>
                    </>
                  )}
                />
